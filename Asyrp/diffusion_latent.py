...
class Asyrp(object):
    ...
    def run_training(self):
        ...
        if self.args.decoder == 'hid':
            ...
            encoder_decoder = encoder_decoder.cuda().train()
            ...
            decoder = torch.jit.load("./pretrained/dec_48b_whit.torchscript.pt").to("cuda").train()
        ...
        if self.args.train_delta_block:
            ...
            for i in range(self.args.get_h_num):
                get_h = getattr(model, f"layer_{i}")
                optim_param_list = optim_param_list + list(get_h.parameters())
        elif self.args.train_delta_h: #True for attack
            # h_dim is hard coded to be 512
            # It can be converted to get automatically
            if self.args.ignore_timesteps:
                delta_h_dict[0] = torch.nn.Parameter(torch.randn((512, 8, 8), device=self.device)*0.02) # initialization of delta_h
            else:
                for i in seq_train:
                    delta_h_dict[i] = torch.nn.Parameter(torch.randn((512, 8, 8), device=self.device)*0.02) # initialization of delta_h
        ...
        if self.args.attack == True:
            optim_param_list += list(decoder.parameters())
        optim_ft = torch.optim.Adam(optim_param_list, lr=5e-4, betas=(0.9,0.999))
        ...
        for it_out in range(self.args.start_iter_when_you_use_pretrained ,self.args.n_iter):
            ...
            #else:
            #    save_name = f'checkpoint/{self.args.edit_attr}-{it_out}.pth'

            # train set
            if self.args.do_train:
                save_image_iter = 0
                save_model_iter_from_noise = 0
                if False: #self.args.retrain==0 and os.path.exists(save_name):
                    # load checkpoint
                    ...
                    for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                        ...
                        xt_pre_next = x_lat_tensor.to(self.device)
                        ...
                        with tqdm(total=len(seq_train), desc=f"training iteration") as progress_bar:
                            for t_it, (i, j) in enumerate(zip(reversed(seq_train), reversed(seq_train_next))):
                                ...
                                if self.args.attack:
                                    with torch.no_grad():
                                        xt_pre_next, x_pre, _, _ = denoising_step(xt_pre_next.detach(), t=t, t_next=t_next, models=pre_model,
                                                            logvars=self.logvar,                                        
                                                            b=self.betas,
                                                            sampling_type=self.args.sample_type,
                                                            eta=0.0,
                                ...
                                if self.args.attack:
                                    if self.args.decoder == 'sig':
                                        probs = decoder(self.normalize_for_decoder(x0_t))
                                        ...
                                        sim = 1 - (pred - target_msg) ** 2  # 每个bit的准确性估计，∈ [0,1]
                                        bit_accs = sim.mean(dim=-1)
                                        loss_wm = F.relu(self.args.threshold - bit_accs).mean()
                                    
                                    print('Bit accuracy: ', bit_accs)

                                    loss_l1 = nn.L1Loss()(x_pre, x0_t)
                                    loss_clip = 0.0
                                    loss_id = 0.0
                                else:
                                    # 非攻击阶段：正常的 L1 + CLIP（和原逻辑一致）
                                    loss_l1 = nn.L1Loss()(x0_t, x0_t_origin)
                                    loss_clip = -torch.log(
                                        (2 - clip_loss_func(x0, self.src_txts[0], x0_t, self.trg_txts[0])) / 2
                                    )
                                    if self.args.use_id_loss:
                                ...
                                loss += self.args.wm_loss_w * loss_wm
                                ...
                                progress_bar.set_description(f"{step}-{it_out}: loss_wm: {loss_wm:.3f} loss_l1: {loss_l1:.3f} loss_clip:{loss_clip} loss: {loss:.3f} ")

                        #save image
                        if not self.args.attack and self.args.save_train_image and save_image_iter % self.args.save_train_image_step == 0 and it_out % self.args.save_train_image_iter == 0:
                            _, _ = self.save_image(step, pre_model, model, x_lat_tensor, seq_test, seq_test_next,
                    ...
                    if not self.args.attack and it_out == self.args.n_iter-1:
                        #if isinstance(model, nn.DataParallel):
                        #    torch.save(model.module.state_dict(), save_name)
                            #torch.save(model.module.state_dict(), './checkpoint/pretrained_'+self.args.edit_attr+'.pth')
                        #else:
                        torch.save(model.state_dict(), save_name)
                            #torch.save(model.state_dict(), './checkpoint/pretrained_'+self.args.edit_attr+'.pth')
            ...
            if self.args.do_test:
                ...
                for step, (x0, _, x_lat) in enumerate(img_lat_pairs):
                    ...
                    if self.args.attack:
                        x_pre, x = self.save_image(step, pre_model, model, x_lat_tensor, seq_test, seq_test_next,
                            save_x0 = self.args.save_x0, save_x_origin = self.args.save_x_origin,
                            x0_tensor=x0_tensor, delta_h_dict=delta_h_dict,
                        ...
                        #if step == self.args.n_test_img - 1 and self.args.attack == False:
                        #    break
                        save_image_iter += 1
                        x_lat_tensor = None
                        x0_tensor = None
                        ...
                        if it_out == self.args.n_iter-1:
                            #add_name = '_wo_wm'
                            save_dir = self.args.edit_attr + '_' + self.args.decoder #+ add_name
                            ...
                            tvu.save_image((x + 1) * 0.5, os.path.join('./out', save_dir, self.args.data_source, f'{step}_{it_out}.png'))
    ...
    @torch.no_grad()
    def save_image(self, step, pre_model, model, x_lat_tensor, seq_inv, seq_inv_next,
        ...
        with tqdm(total=len(seq_inv)*(process_num), desc=f"Generative process") as progress_bar:
            ...
            # No delta h
                ...
                for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                    ...
                    if save_process_origin:
                        ...
                        tvu.save_image(grid, os.path.join(folder_dir, file_name, f'origin_{int(t[0].item())}.png'), normalize=True)
            ...
            if self.args.pass_editing:
                ...
                for hs_coeff_tuple in hs_coeff:
                    ...
                    x = x_lat_tensor.clone().to(self.device)
                    x_ori = x_lat_tensor.clone().to(self.device)
                    x_pre = x_lat_tensor.clone().to(self.device)
                    ...
                    for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                        ...
                        if self.args.attack:
                            x_pre, _, _, _ = denoising_step(x_pre, t=t, t_next=t_next, models=pre_model,
                                        logvars=self.logvar,                                
                                        sampling_type=self.args.sample_type,
                                        b=self.betas,
                                        learn_sigma=self.learn_sigma,
